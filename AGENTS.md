...
- GitHub 공통 소스는 저장소의 `common/app/`입니다. 이것은 기존 동작(plain) 배포본의 소스입니다.
- 클론배치 배포본의 소스는 `common-c/app`이며 `common/app`과 같은 51개 상대 경로를 그대로 가진 별도 트리입니다.
- 두 트리는 서로 독립입니다. 공통 동작을 고치면 양쪽에 각각 반영하고, 반영 후 두 트리를 대조합니다.
- 클론배치 전용 동작은 `common-c/app`에만 반영합니다.
...
- 기존 `v2` 브랜치와 기존 `Grok Chameleon` 워크플로만 사용합니다.
- 새 브랜치, 새 YAML 파일, 새 태그 또는 새 릴리스를 만들지 않습니다.
- 기존 워크플로의 빌드 잡은 `variant` 행렬로 plain과 clone-batch를 각각 한 번씩 돌립니다. 두 변형은 완전히 같은 조립·검사·포장 단계를 지나며 공통 소스 루트만 다릅니다.
- 기존 `v2.0.0` 릴리스의 자산 네 개만 갱신합니다. `Win_Grok_Chameleon_v2.0.zip`, `Mac_Grok_Chameleon_v2.0.zip`은 plain 트리에서, `Win_Grok_Chameleon_C_v2.0.zip`, `Mac_Grok_Chameleon_C_v2.0.zip`은 클론배치 트리에서 만듭니다.
- 두 변형 모두 같은 입력 ZIP(plain 배포본)을 바탕으로 조립하므로 변형끼리 결과가 누적되지 않습니다.
...
- 검증된 네 빌드가 모두 성공한 뒤에만 릴리스 자산을 교체합니다.
- 게시 단계에서 빌드 ZIP 네 개와 GitHub 원격 자산의 SHA-256이 각각 일치해야 하고, 릴리스 자산 수가 정확히 네 개여야 합니다.
- 실제 게시된 Mac ZIP 두 개의 검증 작업까지 성공해야 배포 완료로 판단합니다.

## 중복 최종검사 금지

- 기존 워크플로 전체가 성공한 뒤 같은 Windows·Mac ZIP을 로컬로 다시 내려받아 워크플로와 동일한 검사를 반복하지 않습니다.
- 최종 확인은 워크플로의 `headSha`, 전체 성공 여부, 릴리스 자산 네 개의 이름·크기·SHA-256·갱신 시각, 저장소의 깨끗한 상태로 제한합니다.
