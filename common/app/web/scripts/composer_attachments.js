// Composer attachments
  ...
  async function detailVideoAttachmentForSourceMode(postOverride = null, itemOverride = undefined) {
    ...
    for (const attachment of composerAttachments) {
      if (String(attachment.preview_url || "").startsWith("blob:")) URL.revokeObjectURL(attachment.preview_url);
    }
    }
    ...
    function detailImageForComposer(post, selectedOverride = undefined) {
      ...
      return detailOrderedItems(post).find(isExplicitDetailSourceImage) || null;
    }
  ...
  async function setComposerFiles(files) {
    for (const file of Array.from(files || [])) {
      ...
      composerAttachments.push({
        ...
        data_url: dataUrl,
        preview_url: saved?.preview_url || URL.createObjectURL(file),
