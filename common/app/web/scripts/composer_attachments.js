// Composer attachments
  ...
  async function detailVideoAttachmentForSourceMode(postOverride = null, itemOverride = undefined) {
    ...
    function composerItemIsClonedCopy(node) {
      const metadata = node?.metadata && typeof node.metadata === "object" ? node.metadata : {};
      const imagine = metadata.imagine && typeof metadata.imagine === "object" ? metadata.imagine : {};
      return Boolean(node?.cloned_copy || metadata.cloned_copy || imagine.cloned_copy);
    }

    // A copy reached through Liked carries the same link stamps as the asset it was made
    // from, but it belongs to this account and can be generated from directly. Re-uploading
    // it made a second upload card in Imagine main and severed the result from the card it
    // came from.
    function composerRequiresFreshImagineSource(post, item) {
      if (composerItemIsClonedCopy(item) || composerItemIsClonedCopy(post)) return false;
    ...
    function composerAttachmentMetadataForItem(post, item) {
      ...
      return {
        ...
        cloned_copy: composerItemIsClonedCopy(item) || composerItemIsClonedCopy(post),
