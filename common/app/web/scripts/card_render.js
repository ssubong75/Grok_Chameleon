...
function disposeVirtualCardNode(node) {
  if (!node) return;
  if (typeof disposeCardPreviewNode === "function") {
    disposeCardPreviewNode(node);
    return;
  }
  pauseVirtualCardMedia(node);
}

function replaceVirtualCardListChildren(list, entries, nodeCache) {
  if (!list) return;
  const activeCacheKeys = new Set(entries.map(virtualCardCacheKey).filter(Boolean));
  const disposedNodes = new Set();
  const disposeOnce = (node) => {
    if (!node || disposedNodes.has(node)) return;
    disposedNodes.add(node);
    disposeVirtualCardNode(node);
  };
  const existing = new Map();
  for (const child of list.children) {
    const key = child?.dataset?.cardRenderKey || "";
  ...
  for (const [key, node] of nodeCache?.entries?.() || []) {
...
function virtualCardLayoutMetrics(state, list) {
  const layoutKey = [
    list.clientWidth || 0,
    window.innerWidth || 0,
    window.innerHeight || 0,
    virtualCardListUsesDocumentScroll() ? "document" : "list",
  ].join(":");
  if (state.layoutMetrics?.key === layoutKey) return state.layoutMetrics;
  const styles = getComputedStyle(list);
  state.layoutMetrics = {
    key: layoutKey,
    columns: virtualCardGridColumnCount(list),
    rowHeight: virtualCardGridRowHeight(list),
...
function renderVirtualCardList(listKey, list, entries, options = {}) {
  ...
  if (requestedEntries !== state.entries) state.entries = requestedEntries.filter(Boolean);
  ...
  const layout = virtualCardLayoutMetrics(state, list);
...
function disableVirtualCardList(listKey, list) {
  ...
  if (state) {
    ...
    state.frame = 0;
    for (const node of state.nodeCache?.values?.() || []) disposeVirtualCardNode(node);
...
function virtualCardListPrefetchDistance(list, screens = 2, minimum = 480) {
  if (!list) return Math.max(0, Number(minimum) || 0);
  const viewportHeight = virtualCardListViewport(list).height;
  return Math.max(Number(minimum) || 0, viewportHeight * Math.max(1, Number(screens) || 1));
}
