...
function mergeImagineGeneratedItems(post, generatedItems, options = {}) {
  if (!post) return post;
  const markGeneratedRelationItem = (item) => {
    if (!options.markGeneratedRelation) return item;
    const metadata = item?.metadata && typeof item.metadata === "object" ? item.metadata : {};
    const imagine = metadata.imagine && typeof metadata.imagine === "object" ? metadata.imagine : {};
    return {
      ...item,
  ...
  for (const item of additions) {
    ...
    if (existingIndex < 0) items.push(item);
    else if (options.markGeneratedRelation) {
      items[existingIndex] = markGeneratedRelationItem({ ...items[existingIndex], ...item });
    }
  }
  const representative = options.preserveRepresentative
    ? (post.representative_item || representativeItem(post.items || [], post) || items[0])
    : (representativeItem(items, { ...post, items }) || items[items.length - 1]);
  ...
  return normalizeServerPost({
    ...
    metadata: options.markGeneratedRelation
